ALTER TABLE `erp_settings` ADD `set_focus` TINYINT(1) NOT NULL DEFAULT '0' ;
ALTER TABLE `erp_warehouses_products` ADD `avg_cost` DECIMAL(25,4) NOT NULL ;
...
ALTER TABLE `erp_sales` ADD `return_sale_ref` VARCHAR(55) NULL,
    ADD `sale_id` INT NULL,
    ADD `return_sale_total` DECIMAL(25,4) NOT NULL DEFAULT '0' ;
ALTER TABLE `erp_sale_items` ADD `sale_item_id` INT NULL ;
ALTER TABLE `erp_purchases` ADD `return_purchase_ref` VARCHAR(55) NULL,
    ADD `purchase_id` INT NULL,
    ADD `return_purchase_total` DECIMAL(25,4) NOT NULL DEFAULT '0' ;
ALTER TABLE `erp_purchase_items` ADD `purchase_item_id` INT NULL ;
...
UPDATE `erp_migrations` SET `version` = 315;
UPDATE `erp_settings` SET `version` = '3.0.2.9' where `setting_id` = 1;