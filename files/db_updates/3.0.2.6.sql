ALTER TABLE `erp_settings` ADD `returnp_prefix` varchar(20) DEFAULT NULL;
ALTER TABLE `erp_payments` ADD `approval_code` VARCHAR(50) NULL;
ALTER TABLE `erp_pos_settings` ADD `authorize` TINYINT(1) NULL DEFAULT '0';
ALTER TABLE `erp_purchases` ADD `return_id` int(11) DEFAULT NULL,
  ADD `surcharge` decimal(25,4) NOT NULL DEFAULT '0.00';
ALTER TABLE `erp_quotes` ADD `supplier_id` INT NULL, ADD `supplier` VARCHAR(55) NULL;
ALTER TABLE `erp_order_ref` ADD `rep` INT(11) NOT NULL DEFAULT '1';
...
ALTER TABLE `erp_expenses` ADD `category_id` INT NULL ;
ALTER TABLE `erp_permissions` ADD `products-barcode` TINYINT(1) NOT NULL DEFAULT '0',
  ADD `purchases-return_purchases` TINYINT(1) NOT NULL DEFAULT '0',
  ADD `reports-expenses` TINYINT(1) NOT NULL DEFAULT '0';
UPDATE `erp_migrations` SET `version` = 313;
UPDATE `erp_settings` SET `version` = '3.0.2.6' where `setting_id` = 1;