...
class Shop_settings extends MY_Controller
{
    ...
    function index() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array('shop_name' => DEMO ? 'ERP Shop' : $this->input->post('shop_name'),
                'description' => DEMO ? 'iCloudERP - ERP Shop - Demo Ecommerce Shop that would help you to sell your products from your site. Locked on demo.' : $this->input->post('description'),
    ...
    function delete_page($id = NULL) {
        ...
        if ($this->shop_admin_model->deletePage($id)) {
            $this->erp->send_json(array('error' => 0, 'msg' => lang("page_deleted")));
        }
    }
    ...
    function slugify() {
        ...
        if ($products = $this->shop_admin_model->getAllProducts()) {
            ...
            foreach ($products as $product) {
                $slug = $this->erp->slug($product->name);
