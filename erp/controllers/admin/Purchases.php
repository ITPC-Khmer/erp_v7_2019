...
//generate pdf and force to download
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {
                    ...
                    $total += $this->erp->formatDecimal(($item_net_cost * $item_unit_quantity), 4);
                }
            }
            if (empty($products)) {
    ...
    public function purchase_by_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
                ...
                foreach ($final as $csv_pr) {
                    ...
                    if (isset($csv_pr['code']) && isset($csv_pr['net_unit_cost']) && isset($csv_pr['quantity'])) {
                        ...
                        if ($product_details = $this->purchases_model->getProductByCode($csv_pr['code'])) {
                            ...
                            $item_net_cost = $this->erp->formatDecimal($csv_pr['net_unit_cost']);
                            ...
                            $product_tax += $pr_item_tax;
                            $subtotal = $this->erp->formatDecimal(((($item_net_cost * $item_quantity) + $pr_item_tax) - $pr_item_discount), 4);
                            ...
                            $product = array(
                                ...
                                'date' => date('Y-m-d', strtotime($date)),
                                'status' => $status,
                                'unit_cost' => $this->erp->formatDecimal(($item_net_cost + $item_tax), 4),
                                'real_unit_cost' => $this->erp->formatDecimal(($item_net_cost + $item_tax + $pr_discount), 4),
                            );

                            $products[] = ($product+$gst_data);
                            $total += $this->erp->formatDecimal(($item_net_cost * $item_quantity), 4);
            ...
            $order_tax = $this->site->calculateOrderTax($this->input->post('order_tax'), ($total + $product_tax - $order_discount));
            $total_tax = $this->erp->formatDecimal(($product_tax + $order_tax), 4);
            $grand_total = $this->erp->formatDecimal(($total + $total_tax + $this->erp->formatDecimal($shipping) - $total_discount), 4);
            $data = array('reference_no' => $reference,
                'date' => $date,
                'supplier_id' => $supplier_id,
                'supplier' => $supplier,
    ...
    public function delete($id = null)
    {
        ...
        if ($this->purchases_model->deletePurchase($id)) {
            if ($this->input->is_ajax_request()) {
                $this->erp->send_json(array('error' => 0, 'msg' => lang("purchase_deleted")));
            }
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $this->erp->formatMoney($purchase->grand_total));
                        $row++;
                    }
    ...
    public function email_payment($id = null)
    {
        ...
        $this->data['payment'] = $payment;
        if ( ! $supplier->email) {
            $this->erp->send_json(array('msg' => lang("update_supplier_email")));
        }
        ...
        if ($this->erp->send_email($supplier->email, $subject, $message)) {
    ...
    public function delete_expense($id = null)
    {
        ...
        if ($this->purchases_model->deleteExpense($id)) {
            ...
            $this->erp->send_json(array('error' => 0, 'msg' => lang("expense_deleted")));
        }
    }
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel') {
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $this->erp->formatMoney($expense->amount));
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {
                    ...
                    $product_tax += $pr_item_tax;
                    $subtotal = $this->erp->formatDecimal((($item_net_cost * $item_unit_quantity) + $pr_item_tax), 4);
                    $unit = $this->site->getUnitByID($item_unit);
