...
class Customers extends MY_Controller
{
    ...
    function import_csv()
    {
        $this->erp->checkPermissions('add', true);
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["csv_file"])) {
                ...
                // $this->erp->print_arrays($data, $updated);
            }
    ...
    function delete($id = NULL)
    {
        ...
        if ($this->input->get('id') == 1) {
            $this->erp->send_json(array('error' => 1, 'msg' => lang("customer_x_deleted")));
        }

        if ($this->companies_model->deleteCustomer($id)) {
            $this->erp->send_json(array('error' => 0, 'msg' => lang("customer_deleted")));
        } else {
            $this->erp->send_json(array('error' => 1, 'msg' => lang("customer_x_deleted_have_sales")));
        }
    }
    ...
    function suggestions($term = NULL, $limit = NULL, $a = NULL)
    {
        ...
        if ($a) {
            $this->erp->send_json($result);
        }
        $rows['results'] = $result;
        $this->erp->send_json($rows);
    }
    ...
    function getCustomer($id = NULL)
    {
        ...
        $this->erp->send_json(array(array('id' => $row->id, 'text' => ($row->company != '-' ? $row->company : $row->name))));
    }

    function get_customer_details($id = NULL)
    {
        $this->erp->send_json($this->companies_model->getCompanyByID($id));
    }
    ...
    function get_award_points($id = NULL)
    {
        $this->erp->checkPermissions('index');
        $row = $this->companies_model->getCompanyByID($id);
        $this->erp->send_json(array('ca_points' => $row->award_points));
    }
    ...
    public function delete_deposit($id)
    {
        ...
        if ($this->companies_model->deleteDeposit($id)) {
            $this->erp->send_json(array('error' => 0, 'msg' => lang("deposit_deleted")));
        }
    }
