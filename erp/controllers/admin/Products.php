...
class Products extends MY_Controller
{
    ...
    function barcode($product_code = NULL, $bcs = 'code128', $height = 40)
    {
        ...
        echo $this->erp->barcode($product_code, $bcs, $height, true, false, true);
    }
            ...
            for ($m = 0; $m < $s; $m++) {
                ...
                if ($variants = $this->products_model->getProductOptions($pid)) {
                    foreach ($variants as $option) {
                        if ($this->input->post('vt_'.$product->id.'_'.$option->id)) {
                            $barcodes[] = array(
                                ...
                                'price' => $this->input->post('price') ?  $this->erp->formatMoney($option->price != 0 ? ($product->price+$option->price) : $product->price, 'none') : FALSE,
                    ...
                    $barcodes[] = array(
                        ...
                        'bcis' => $bci_size,
                        'price' => $this->input->post('price') ?  $this->erp->formatMoney($product->price, 'none') : FALSE,
    ...
    function add($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'start_date' => $this->input->post('start_date') ? $this->erp->fsd($this->input->post('start_date')) : NULL,
                'end_date' => $this->input->post('end_date') ? $this->erp->fsd($this->input->post('end_date')) : NULL,
    ...
    function addByAjax()
    {
        ...
        if ($this->input->get('token') && $this->input->get('token') == $this->session->userdata('user_csrf') && $this->input->is_ajax_request()) {
            ...
            if ($row = $this->products_model->addAjaxProduct($product)) {
                ...
                $this->erp->send_json(array('msg' => 'success', 'result' => $pr));
            } else {
    ...
    function delete($id = NULL)
    {
        ...
        if ($this->products_model->deleteProduct($id)) {
            if($this->input->is_ajax_request()) {
                $this->erp->send_json(array('error' => 0, 'msg' => lang("product_deleted")));
            }
    ...
    function delete_adjustment($id = NULL)
    {
        ...
        if ($this->products_model->deleteAdjustment($id)) {
            $this->erp->send_json(array('error' => 0, 'msg' => lang("adjustment_deleted")));
        }
    ...
    public function delete_image($id = NULL)
    {
        ...
        if ($id && $this->input->is_ajax_request()) {
            ...
            $this->erp->send_json(array('error' => 0, 'msg' => lang("image_deleted")));
        }
        $this->erp->send_json(array('error' => 1, 'msg' => lang("ajax_error")));
    }
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        if ($items) {
                            foreach ($items as $item) {
                                $products .= $item->product_name.'('.$this->erp->formatQuantity($item->type == 'subtraction' ? -$item->quantity : $item->quantity).')'."\n";
                            }
                        }

                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->erp->hrld($adjustment->date));
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $this->erp->decode_html($adjustment->note));
    ...
    function view_count($id)
    {
        ...
        if ( ! $stock_count->finalized) {
            $this->erp->md('admin/products/finalize_count/'.$id);
        }
