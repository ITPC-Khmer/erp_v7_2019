...
class Pos extends MY_Controller
{
    ...
    public function index($sid = NULL)
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($this->pos_settings->rounding) {
                $round_total = $this->erp->roundNumber($grand_total, $this->pos_settings->rounding);
                $rounding = $this->erp->formatMoney($round_total - $grand_total);
            }
    ...
    public function ajaxcategorydata($category_id = NULL)
    {
        ...
        $this->erp->send_json(array('products' => $products, 'subcategories' => $scats, 'tcp' => $tcp));
    }
    ...
    public function ajaxbranddata($brand_id = NULL)
    {
        ...
        $this->erp->send_json(array('products' => $products, 'tcp' => $tcp));
    }
    ...
    public function check_pin()
    {
        ...
        if ($pin == $this->pos_pin) {
            $this->erp->send_json(array('res' => 1));
        }
        $this->erp->send_json(array('res' => 0));
    }
    ...
    public function opened_bills($per_page = 0)
    {
        ...
        if (!empty($bills)) {
            ...
            foreach ($bills as $bill) {
                $html .= '<li><button type="button" class="btn btn-info sus_sale" id="' . $bill->id . '"><p>' . $bill->suspend_note . '</p><strong>' . $bill->customer . '</strong><br>'.lang('date').': ' . $bill->date . '<br>'.lang('items').': ' . $bill->count . '<br>'.lang('total').': ' . $this->erp->formatMoney($bill->total) . '</button></li>';
            }
            $html .= '</ul>';
        } else {
    ...
    public function delete($id = NULL)
    {
        ...
        if ($this->pos_model->deleteBill($id)) {
            $this->erp->send_json(array('error' => 0, 'msg' => lang("suspended_sale_deleted")));
        }
    }
    ...
    public function email_receipt($sale_id = NULL, $view = null)
    {
        ...
        if (!$to) {
            $this->erp->send_json(array('msg' => $this->lang->line("no_meil_provided")));
        }

        try {
            if ($this->erp->send_email($to, lang('receipt_from') .' ' . $this->data['biller']->company, $receipt)) {
                $this->erp->send_json(array('msg' => $this->lang->line("email_sent")));
            } else {
                $this->erp->send_json(array('msg' => $this->lang->line("email_failed")));
            }
        } catch (Exception $e) {
            $this->erp->send_json(array('msg' => $e->getMessage()));
        }
    ...
    function delete_printer($id = NULL)
    {
        ...
        if ($this->pos_model->deletePrinter($id)) {
            $this->erp->send_json(array('error' => 0, 'msg' => lang("printer_deleted")));
        }
