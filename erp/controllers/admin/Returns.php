...
class Returns extends MY_Controller
{
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == true) {
            $date = ($this->Owner || $this->Admin) ? $this->erp->fld(trim($this->input->post('date'))) : date('Y-m-d H:i:s');
            ...
            $grand_total = $this->erp->formatDecimal(($total + $total_tax - $order_discount), 4);
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            $date = ($this->Owner || $this->Admin) ? $this->erp->fld(trim($this->input->post('date'))) : $inv->date;
    ...
    public function delete($id = null)
    {
        ...
        if ($this->returns_model->deleteReturn($id)) {
            if ($this->input->is_ajax_request()) {
                $this->erp->send_json(array('error' => 0, 'msg' => lang("return_deleted")));
            }
