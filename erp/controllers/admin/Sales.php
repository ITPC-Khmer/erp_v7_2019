...
class Sales extends MY_Controller
{
    ...
    public function email($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            try {
                if ($this->erp->send_email($to, $subject, $message, null, null, $attachment, $cc, $bcc)) {
    ...
    public function add($quote_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $total_discount = $this->erp->formatDecimal(($order_discount + $product_discount), 4);
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    $item_net_price = $unit_price;
                    $pr_item_discount = $this->erp->formatDecimal($pr_discount * $item_unit_quantity, 4);
                    $product_discount += $pr_item_discount;
                    $pr_item_tax = $item_tax = 0;
                    $tax = "";
                    ...
                    $product_tax += $pr_item_tax;
                    $subtotal = $this->erp->formatDecimal((($item_net_price * $item_unit_quantity) + $pr_item_tax), 4);
            ...
            $order_tax = $this->site->calculateOrderTax($this->input->post('order_tax'), ($total + $product_tax - $order_discount));
            $total_tax = $this->erp->formatDecimal($product_tax + $order_tax, 4);
            $grand_total = $this->erp->formatDecimal(($total + $total_tax + $this->erp->formatDecimal($return_surcharge) - $order_discount), 4);
            $data = array('date' => $date,
                'sale_id' => $id,
            ...
            // $this->erp->print_arrays($data, $products, $si_return, $payment);
        }
    ...
    public function delete($id = null)
    {
        ...
        if ($inv->sale_status == 'returned') {
            $this->erp->send_json(array('error' => 1, 'msg' => lang("sale_x_action")));
        }

        if ($this->sales_model->deleteSale($id)) {
            if ($this->input->is_ajax_request()) {
                $this->erp->send_json(array('error' => 0, 'msg' => lang("sale_deleted")));
            }
    ...
    public function delete_return($id = null)
    {
        ...
        if ($this->sales_model->deleteReturn($id)) {
            if ($this->input->is_ajax_request()) {
                $this->erp->send_json(array('error' => 0, 'msg' => lang("return_sale_deleted")));
            }
    ...
    public function delete_delivery($id = null)
    {
        ...
        if ($this->sales_model->deleteDelivery($id)) {
            $this->erp->send_json(array('error' => 0, 'msg' => lang("delivery_deleted")));
        }
    ...
    public function delivery_actions()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    $this->erp->checkPermissions('delete_delivery');
                    foreach ($_POST['val'] as $id) {
    ...
    public function email_payment($id = null)
    {
        ...
        $customer = $this->site->getCompanyByID($inv->customer_id);
        if ( ! $customer->email) {
            $this->erp->send_json(array('msg' => lang("update_customer_email")));
        }
        $this->data['inv'] = $inv;
        $this->data['payment'] = $payment;
        ...
        if ($this->erp->send_email($customer->email, $subject, $message)) {
            $this->erp->send_json(array('msg' => lang("email_sent")));
        } else {
            $this->erp->send_json(array('msg' => lang("email_failed")));
        }
    }
    ...
    {
        $this->erp->checkPermissions('add_gift_card', true);
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->input->post('expiry')) {
                $card_data['expiry'] = $this->erp->fld(trim($this->input->post('expiry')));
            }
        ...
        if ($gc = $this->site->getGiftCardByNO($no)) {
            if ($gc->expiry) {
                if ($gc->expiry >= date('Y-m-d')) {
                    $this->erp->send_json($gc);
                } else {
                    $this->erp->send_json(false);
                }
            } else {
                $this->erp->send_json($gc);
            }
    ...
    public function add_gift_card()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            // $this->erp->print_arrays($data, $ca_data, $sa_data);
    ...
    public function sell_gift_card()
    {
        ...
        if (!$error) {
            if ($this->sales_model->addGiftCard($data)) {
                $this->erp->send_json(array('result' => 'success', 'message' => lang("gift_card_added")));
            }
        } else {
            $this->erp->send_json(array('result' => 'failed', 'message' => $error));
        }
    ...
    public function delete_gift_card($id = null)
    {
        ...
        if ($this->sales_model->deleteGiftCard($id)) {
            $this->erp->send_json(array('error' => 0, 'msg' => lang("gift_card_deleted")));
        }
    }
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    $this->erp->checkPermissions('delete_gift_card');
                    foreach ($_POST['val'] as $id) {
    ...
    public function get_award_points($id = null)
    {
        ...
        $this->erp->send_json(array('sa_points' => $row->award_points));
    }
                ...
                foreach ($final as $csv_pr) {
                    ...
                    if (isset($csv_pr['code']) && isset($csv_pr['net_unit_price']) && isset($csv_pr['quantity'])) {
                        ...
                        if ($product_details = $this->sales_model->getProductByCode($csv_pr['code'])) {
                            ...
                            $item_net_price = $this->erp->formatDecimal($csv_pr['net_unit_price']);
                            ...
                            if (isset($item_code) && isset($item_net_price) && isset($item_quantity)) {
                                ...
                                $item_net_price = $this->erp->formatDecimal(($item_net_price - $pr_discount), 4);
                                $pr_item_discount = $this->erp->formatDecimal(($pr_discount * $item_quantity), 4);
                                $product_discount += $pr_item_discount;
                                ...
                                $product_tax += $pr_item_tax;
                                $subtotal = $this->erp->formatDecimal((($item_net_price * $item_quantity) + $pr_item_tax), 4);
                                ...
                                $product = array(
                                    ...
                                    'subtotal' => $subtotal,
                                    'serial_no' => $item_serial,
                                    'unit_price' => $this->erp->formatDecimal(($item_net_price + $item_tax), 4),
                                    'real_unit_price' => $this->erp->formatDecimal(($item_net_price + $item_tax + $pr_discount), 4),
                                );

                                $products[] = ($product+$gst_data);
                                $total += $this->erp->formatDecimal(($item_net_price * $item_quantity), 4);
                            }
