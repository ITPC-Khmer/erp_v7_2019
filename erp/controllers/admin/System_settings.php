...
class system_settings extends MY_Controller
{
    ...
    function index()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ((file_exists(APPPATH.'language'.DIRECTORY_SEPARATOR.$language.DIRECTORY_SEPARATOR.'admin'.DIRECTORY_SEPARATOR.'erp_lang.php') && is_dir(APPPATH.DIRECTORY_SEPARATOR.'language'.DIRECTORY_SEPARATOR.$language)) || $language == 'english') {
                $lang = $language;
            } else {
            ...
            $data = array('site_name' => DEMO ? 'iCloudERP' : $this->input->post('site_name'),
                ...
                'default_email' => DEMO ? 'noreply@erp.CloudNET.my' : $this->input->post('email'),
                'language' => $lang,
    ...
    function updates()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $updates = get_remote_contents($protocol.'api.cloudnet.com.kh/v1/update/', $fields);
    ...
    function delete_currency($id = NULL)
    {
        ...
        if ($this->settings_model->deleteCurrency($id)) {
            $this->erp->send_json(array('error' => 0, 'msg' => lang("currency_deleted")));
        }
    }
    ...
    function delete_category($id = NULL)
    {
        ...
        if ($this->site->getSubCategories($id)) {
            $this->erp->send_json(array('error' => 1, 'msg' => lang("category_has_subcategory")));
        }

        if ($this->settings_model->deleteCategory($id)) {
            $this->erp->send_json(array('error' => 0, 'msg' => lang("category_deleted")));
        }
    }
    ...
    function delete_tax_rate($id = NULL)
    {
        if ($this->settings_model->deleteTaxRate($id)) {
            $this->erp->send_json(array('error' => 0, 'msg' => lang("tax_rate_deleted")));
        }
    }
    ...
    function delete_customer_group($id = NULL)
    {
        if ($this->settings_model->deleteCustomerGroup($id)) {
            $this->erp->send_json(array('error' => 0, 'msg' => lang("customer_group_deleted")));
        }
    }
    ...
    function delete_warehouse($id = NULL)
    {
        if ($this->settings_model->deleteWarehouse($id)) {
            $this->erp->send_json(array('error' => 0, 'msg' => lang("warehouse_deleted")));
        }
    }
    ...
    function delete_variant($id = NULL)
    {
        if ($this->settings_model->deleteVariant($id)) {
            $this->erp->send_json(array('error' => 0, 'msg' => lang("variant_deleted")));
        }
    }
    ...
    function delete_expense_category($id = NULL)
    {
        ...
        if ($this->settings_model->hasExpenseCategoryRecord($id)) {
            $this->erp->send_json(array('error' => 1, 'msg' => lang("category_has_expenses")));
        }

        if ($this->settings_model->deleteExpenseCategory($id)) {
            $this->erp->send_json(array('error' => 0, 'msg' => lang("expense_category_deleted")));
        }
    }
    ...
    function import_categories()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            // $this->erp->print_arrays($categories, $subcategories);
        }
    ...
    function delete_unit($id = NULL)
    {
        ...
        if ($this->settings_model->getUnitChildren($id)) {
            $this->erp->send_json(array('error' => 1, 'msg' => lang("unit_has_subunit")));
        }

        if ($this->settings_model->deleteUnit($id)) {
            $this->erp->send_json(array('error' => 0, 'msg' => lang("unit_deleted")));
        }
    }
    ...
    function delete_price_group($id = NULL)
    {
        if ($this->settings_model->deletePriceGroup($id)) {
            $this->erp->send_json(array('error' => 0, 'msg' => lang("price_group_deleted")));
        }
    }
    ...
    function delete_brand($id = NULL)
    {
        ...
        if ($this->settings_model->brandHasProducts($id)) {
            $this->erp->send_json(array('error' => 1, 'msg' => lang("brand_has_products")));
        }

        if ($this->settings_model->deleteBrand($id)) {
            $this->erp->send_json(array('error' => 0, 'msg' => lang("brand_deleted")));
        }
    }
