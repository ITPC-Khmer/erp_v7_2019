...
class PHPExcel_Shared_OLE_PPS_Root extends PHPExcel_Shared_OLE_PPS
	...
	*/
	public function save($filename)
	{
		...
		$this->_ERPLL_BLOCK_SIZE= pow(2,
					  ((isset($this->_ERPLL_BLOCK_SIZE))?  self::_adjust2($this->_ERPLL_BLOCK_SIZE): 6));
	...
	public function _calcSize(&$raList)
	{
		...
		for ($i = 0; $i < $iCount; ++$i) {
			if ($raList[$i]->Type == PHPExcel_Shared_OLE::OLE_PPS_TYPE_FILE) {
				...
				if ($raList[$i]->Size < PHPExcel_Shared_OLE::OLE_DATA_SIZE_ERPLL) {
					$iSBcnt += floor($raList[$i]->Size / $this->_ERPLL_BLOCK_SIZE)
								  + (($raList[$i]->Size % $this->_ERPLL_BLOCK_SIZE)? 1: 0);
				} else {
		...
		$iSmallLen = $iSBcnt * $this->_ERPLL_BLOCK_SIZE;
	...
	*/
	...
	/**
	* Saving big data (PPS's with data bigger than PHPExcel_Shared_OLE::OLE_DATA_SIZE_ERPLL)
	*
	* @access public
	...
	*/
	...
	/**
	* get small data (PPS's with data smaller than PHPExcel_Shared_OLE::OLE_DATA_SIZE_ERPLL)
	*
	* @access public
	* @param array &$raList Reference to array of PPS's
	*/
	public function _makeSmallData(&$raList)
	{
		...
		for ($i = 0; $i < $iCount; ++$i) {
			...
			if ($raList[$i]->Type == PHPExcel_Shared_OLE::OLE_PPS_TYPE_FILE) {
				...
				if ($raList[$i]->Size < PHPExcel_Shared_OLE::OLE_DATA_SIZE_ERPLL) {
					$iSmbCnt = floor($raList[$i]->Size / $this->_ERPLL_BLOCK_SIZE)
								  + (($raList[$i]->Size % $this->_ERPLL_BLOCK_SIZE)? 1: 0);
					// Add to SBD
					$jB = $iSmbCnt - 1;
					for ($j = 0; $j < $jB; ++$j) {
					...
					//} else {
						$sRes .= $raList[$i]->_data;
					//}
					if ($raList[$i]->Size % $this->_ERPLL_BLOCK_SIZE) {
						$sRes .= str_repeat("\x00",$this->_ERPLL_BLOCK_SIZE - ($raList[$i]->Size % $this->_ERPLL_BLOCK_SIZE));
					}
					// Set for PPS
