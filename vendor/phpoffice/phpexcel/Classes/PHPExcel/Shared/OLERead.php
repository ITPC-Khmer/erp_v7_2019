...
class PHPExcel_Shared_OLERead {
	...
	const ERPLL_BLOCK_SIZE					= 0x40;

	// Size of a directory entry always = 128 bytes
	const PROPERTY_STORAGE_BLOCK_SIZE		= 0x80;

	// Minimum size of a standard stream = 4096 bytes, streams smaller than this are stored as short streams
	const ERPLL_BLOCK_THRESHOLD				= 0x1000;

	// header offsets
	const NUM_BIG_BLOCK_DEPOT_BLOCKS_POS	= 0x2c;
	const ROOT_START_BLOCK_POS				= 0x30;
	const ERPLL_BLOCK_DEPOT_BLOCK_POS		= 0x3c;
	...
	public function read($sFileName)
	{
		...
		$this->sbdStartBlock = self::_GetInt4d($this->data, self::ERPLL_BLOCK_DEPOT_BLOCK_POS);
	...
	public function getStream($stream)
	{
		...
		if ($this->props[$stream]['size'] < self::ERPLL_BLOCK_THRESHOLD) {
			$rootdata = $this->_readData($this->props[$this->rootentry]['startBlock']);

			$block = $this->props[$stream]['startBlock'];

			while ($block != -2) {
	  			$pos = $block * self::ERPLL_BLOCK_SIZE;
				$streamData .= substr($rootdata, $pos, self::ERPLL_BLOCK_SIZE);
